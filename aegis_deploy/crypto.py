...
# NOTE: `cryptography` is imported lazily inside the functions that need it.
# Its OpenSSL/Rust extension is slow to load on the shared host's CPU-limited
# environment and was blowing past LiteSpeed's app-spawn timeout when imported
# at module load. Deferring it lets the app boot instantly; the heavy import
# happens once, on the first request that actually verifies a signature.
