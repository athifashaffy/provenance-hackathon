...
from db import get_pool, init_schema, POOL
...
# The Passenger/a2wsgi WSGI bridge does not emit ASGI lifespan events, so the
# @app.on_event("startup") handler below never fires in production. Bind the
# DB pool to app.state eagerly at import time so the integrity middleware can
# always reach it; schema is created lazily on first query (see db.py).
app.state.pool = POOL
...
async def api_qr(product_id: str, request: Request):
    ...
    import qrcode  # lazy: keeps Pillow/qrcode off the cold-start import path
