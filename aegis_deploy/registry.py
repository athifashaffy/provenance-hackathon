...
async def get_supplier(pool: object, supplier_id: str) -> dict | None:
...
async def list_suppliers(pool: object) -> list[dict]:
    async with pool.acquire() as conn:
        rows = await conn.fetch("SELECT * FROM suppliers ORDER BY name")
        return [dict(r) for r in rows]


async def register_supplier(pool: object, supplier_id: str, name: str,
